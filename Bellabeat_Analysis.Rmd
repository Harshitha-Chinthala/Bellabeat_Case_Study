...
# Introduction
This report presents a detailed analysis of smart device usage data to help Bellabeat optimize their marketing strategy. The focus is on analyzing Fitbit data to understand user behavior and activity trends.

# Business Task
Analyze trends in physical activity, sleep, and heart rate data collected from Fitbit users and identify actionable insights that Bellabeat can use to improve marketing and product decisions.

# Data Distribution
The data was collected from 30 FitBit users and includes:

* Daily activity data(steps, distance, calories, etc.)
* Sleep data(duration, time in bed)
* Heart rate data(by second)
...
## 9. Average Heart Rate by Weekday
```{r}
...
## 11. Time in Bed vs Calories Burned
```{r}
ggplot(data = combined_data) +
...
# Recommendations
* Encourage users to walk more during low-activity days.
* Introduce reminders and sleep tips in the app based on sleep trends.
* Use heart rate + activity insights to tailor fitness plans.
* Focus marketing on days when users are most active.

# Conclusion
The Fitbit data reveals user patterns in activity, sleep, and heart rate that Bellabeat can use to improve customer engagement and personalize features in their wellness app.
